package etherjava.domain.repository.account;
...
public class AccountRepository {
	...
	private final HTable hTableAccount;
	...
	public AccountRepository() throws IOException {
	   ...
	   hTableAccount = (HTable) connection.getTable(TableName.valueOf("etherjava:account"));
	}
	...
	public void save(String addressToString, double balance, int nonce) throws IOException {
		...
		hTableAccount.put(put);
	}

	public Account findOne(String addressToString) {
		try {
			Result getResult = hTableAccount.get(new Get(Bytes.toBytes(addressToString)));
