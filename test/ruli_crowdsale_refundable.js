...
import { RuliCrowdsale, cap, rate, initialRuliFundBalance, goal, BigNumber, setTimeingToBaseTokenRate } from './helpers/ruli_helper';
...
contract('RuliCrowdsale', ([owner, wallet, investor, notInvestor]) => {
  ...
  beforeEach(async function () {
    ...
    this.crowdsale = await RuliCrowdsale.new(this.startBlock, this.endBlock,
      rate.base, wallet, cap, initialRuliFundBalance, ether(goal),
  ...
  describe('creating a valid refundable crowdsale', () => {
    it('should fail with zero goal', async function () {
      await RuliCrowdsale.new(this.startBlock, this.endBlock, rate.base, wallet, cap, initialRuliFundBalance, 0,
      rate.preSale, rate.week1, rate.week2, rate.week3, { from: owner })
      .should.be.rejectedWith(EVMThrow);
    });

    it('should goal be 700 ETH', async function () {
    ...
    it('should goal unit be wei(not ether)', async () => {
      const target = await RuliCrowdsale.deployed();
  ...
  describe('deny refunds', () => {
    ...
    it('should deny refunds if cap was reached', async function () {
      await advanceToBlock(this.startBlock - 1);
      // 150000000 / 2000 = 75000
      const capReachingAmount = await ether(75000);
  ...
  describe('allow refunds', () => {
    it('should allow refunds after end if goal was not reached', async function () {
      ...
      await advanceToBlock(this.startBlock - 1);
      await this.crowdsale.sendTransaction({ value: lessThanGoal, from: investor, gasPrice: 155000 });
      await advanceToBlock(this.endBlock);
      await this.crowdsale.finalize({ from: owner });

      const sent = web3.eth.getBalance(investor);
      // TODO: gasPrice
      await this.crowdsale.claimRefund({ from: investor, gasPrice: 155000 })
        .should.be.fulfilled;
    ...
    it('should return 0 ether to non investors', async function () {
      ...
      await this.crowdsale.claimRefund({ from: notInvestor, gasPrice: 155000 }).should.be.fulfilled;
